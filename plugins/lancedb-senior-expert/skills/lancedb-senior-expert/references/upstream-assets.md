...
## Version anchor
The `ml-lancedb` SDK source read for these catalogs was around Rust/core `0.28.0-beta.4`; docs snippets track released Python `lancedb==0.30.0`. Where the imperative API and the async config classes differ (e.g. `create_index` static defaults vs the `IvfPq` data-derived defaults), the catalogs flag it ⚠️. Re-verify the installed SDK version at build.
